...
void model:: create_shapes(std::vector<Rule*>rules)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (rules.at(i)->get_obj_name()!="")
		{
			...
			//std::cout << a->get_rotatey() << std::endl;
			//if (a->get_rotatey() != 0)
			//{
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			if (a->get_pairwise_distances().at(k).second > 0)
			{
				Rule*pair_rule = get_rule(a->get_pairwise_distances().at(k).first,rules);
